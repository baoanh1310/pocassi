...
const FooterLinks = ({ router, heading, items, extraClasses }) => (
  <div className={`flex-1 justify-start items-start ${extraClasses}`}>
    <h3 className="font-poppins dark:text-white text-nft-black-1 font-semibold text-xl mb-10">{heading}</h3>
    {
      items.map((item, index) => (
        ...
        >
          <span onClick={() => {
            const to = item.toLowerCase().split(' ').join('-');
            switch (to) {
              case 'how-it-works': router.push(to); break;
              default: router.push('/');
            }
          }}
          >{item}
      ...
      <div className="w-full minmd:w-4/5 flex flex-row md:flex-col sm:px-4 px-16">
        <div className="flexStart flex-1 flex-col">
          <div className="flexCenter cursor-pointer">
            ...
            <p className=" dark:text-white text-nft-dark font-semibold text-lg ml-1">Pocassi</p>
          </div>
        ...
        <div className="flex-1 flexBetweenStart flex-wrap ml-10 md:ml-0 md:mt-8">
          <FooterLinks router={router} heading="Pocassi" items={['Explore', 'How it Works', 'Contact Us']} />
          <FooterLinks router={router} heading="Support" items={['Help Center', 'Terms of service', 'Legal', 'Privacy policy']} extraClasses="ml-4" />
        </div>
      </div>
      <div className="flexCenter w-full mt-5 border-t dark:border-nft-black-1 border-nft-gray-1 sm:px-4 px-16">
        <div className="flexBetween flex-row w-full minmd:w-4/5 sm:flex-col mt-7">
          <p className="font-poppins dark:text-white text-nft-black-1 font-semibold text-base">Pocassi, Inc. All Rights Reserved</p>
